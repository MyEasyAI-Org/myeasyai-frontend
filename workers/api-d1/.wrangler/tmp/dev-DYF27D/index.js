...
// .wrangler/tmp/bundle-NRPuq4/checked-fetch.js
...
// .wrangler/tmp/bundle-NRPuq4/strip-cf-connecting-ip-header.js
...
__export(schema_exports, {
  contentBusinessProfiles: () => contentBusinessProfiles,
...
var sites = sqliteTable("sites", {
  ...
  // JSON string com configurações do site
  created_at: text("created_at").default(sql`(datetime('now'))`),
  updated_at: text("updated_at"),
...
var contentBusinessProfiles = sqliteTable("content_business_profiles", {
  id: text("id").primaryKey(),
  user_id: text("user_id").notNull().references(() => users.uuid, { onDelete: "cascade" }),
  name: text("name").notNull(),
  // "Restaurante do João", "Loja de Instrumentos"
  business_niche: text("business_niche").notNull(),
  // 'restaurant', 'retail', etc.
  target_audience: text("target_audience"),
  brand_voice: text("brand_voice").default("professional"),
  selected_networks: text("selected_networks"),
  // JSON array
  preferred_content_types: text("preferred_content_types"),
  // JSON array
  icon: text("icon"),
  // Emoji ou identificador de ícone
...
var contentLibrary = sqliteTable("content_library", {
  id: text("id").primaryKey(),
  user_id: text("user_id").notNull().references(() => users.uuid, { onDelete: "cascade" }),
  profile_id: text("profile_id").notNull().references(() => contentBusinessProfiles.id, { onDelete: "cascade" }),
  content_type: text("content_type").notNull(),
  // 'feed_post', 'caption', etc.
...
var contentCalendar = sqliteTable("content_calendar", {
  id: text("id").primaryKey(),
  user_id: text("user_id").notNull().references(() => users.uuid, { onDelete: "cascade" }),
  profile_id: text("profile_id").notNull().references(() => contentBusinessProfiles.id, { onDelete: "cascade" }),
...
contentRoutes.get("/profiles/:id", async (c) => {
  ...
  const parsedProfile = {
    ...
    preferred_content_types: profile.preferred_content_types ? JSON.parse(profile.preferred_content_types) : []
  };
...
contentRoutes.post("/profiles", async (c) => {
  ...
  const newProfile = {
    ...
    preferred_content_types: body.preferred_content_types ? JSON.stringify(body.preferred_content_types) : null,
    icon: body.icon || null,
...
contentRoutes.patch("/profiles/:id", async (c) => {
  ...
  if (selected_networks !== void 0) {
    updateData.selected_networks = Array.isArray(selected_networks) ? JSON.stringify(selected_networks) : selected_networks;
  }
  if (preferred_content_types !== void 0) {
    updateData.preferred_content_types = Array.isArray(preferred_content_types) ? JSON.stringify(preferred_content_types) : preferred_content_types;
  }
  const result = await db.update(contentBusinessProfiles).set(updateData).where(eq(contentBusinessProfiles.id, id)).returning();
  if (result.length === 0) {
    return c.json({ error: "Profile not found" }, 404);
  }
  const parsedProfile = {
    ...
    preferred_content_types: result[0].preferred_content_types ? JSON.parse(result[0].preferred_content_types) : []
  };
...
contentRoutes.patch("/profiles/:id/set-default", async (c) => {
  ...
  await db.update(contentBusinessProfiles).set({ is_default: false, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(contentBusinessProfiles.user_id, profile.user_id));
  const result = await db.update(contentBusinessProfiles).set({ is_default: true, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(contentBusinessProfiles.id, id)).returning();
  const parsedProfile = {
    ...result[0],
    selected_networks: result[0].selected_networks ? JSON.parse(result[0].selected_networks) : [],
    preferred_content_types: result[0].preferred_content_types ? JSON.parse(result[0].preferred_content_types) : []
...
contentRoutes.delete("/profiles/:id", async (c) => {
  const db = c.get("db");
  const id = c.req.param("id");
  const result = await db.delete(contentBusinessProfiles).where(eq(contentBusinessProfiles.id, id)).returning();
  if (result.length === 0) {
    return c.json({ error: "Profile not found" }, 404);
  }
...
contentRoutes.get("/library/profile/:profileId", async (c) => {
  ...
  if (contentType) {
    filteredContents = filteredContents.filter((c2) => c2.content_type === contentType);
  }
  if (network) {
    filteredContents = filteredContents.filter((c2) => c2.network === network);
  }
  const parsedContents = filteredContents.map((c2) => ({
    ...c2,
    hashtags: c2.hashtags ? JSON.parse(c2.hashtags) : [],
    variations: c2.variations ? JSON.parse(c2.variations) : [],
    tags: c2.tags ? JSON.parse(c2.tags) : []
  }));
...
contentRoutes.patch("/library/:id", async (c) => {
  ...
  const result = await db.update(contentLibrary).set(updateData).where(eq(contentLibrary.id, id)).returning();
  if (result.length === 0) {
    return c.json({ error: "Content not found" }, 404);
  }
  const parsedContent = {
...
contentRoutes.patch("/library/:id/toggle-favorite", async (c) => {
  ...
  const result = await db.update(contentLibrary).set({ is_favorite: !content.is_favorite }).where(eq(contentLibrary.id, id)).returning();
  const parsedContent = {
    ...result[0],
    hashtags: result[0].hashtags ? JSON.parse(result[0].hashtags) : [],
    variations: result[0].variations ? JSON.parse(result[0].variations) : [],
...
contentRoutes.post("/calendar", async (c) => {
  const db = c.get("db");
  const body = await c.req.json();
  if (!body.user_id || !body.profile_id || !body.scheduled_date || !body.network || !body.content_type || !body.title) {
    return c.json(
      {
        error: "user_id, profile_id, scheduled_date, network, content_type, and title are required"
      },
      400
    );
  }
...
contentRoutes.post("/calendar/bulk", async (c) => {
  ...
  const now2 = (/* @__PURE__ */ new Date()).toISOString();
  const days = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
  const newEntries = body.entries.map((entry) => {
...
contentRoutes.patch("/calendar/:id", async (c) => {
  ...
  const result = await db.update(contentCalendar).set(updateData).where(eq(contentCalendar.id, id)).returning();
  if (result.length === 0) {
    return c.json({ error: "Calendar entry not found" }, 404);
  }
  const parsedEntry = {
...
contentRoutes.delete("/calendar/profile/:profileId", async (c) => {
  const db = c.get("db");
  const profileId = c.req.param("profileId");
  const result = await db.delete(contentCalendar).where(eq(contentCalendar.profile_id, profileId)).returning();
...
// .wrangler/tmp/bundle-NRPuq4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NRPuq4/middleware-loader.entry.ts
