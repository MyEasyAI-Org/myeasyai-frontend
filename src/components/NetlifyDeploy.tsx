...
export function NetlifyDeploy({
  ...
  if (deployResult) {
    return (
      <div
        className={`rounded-lg border border-green-700 bg-green-900/20 p-6 space-y-4 ${className}`}
      >
        {/* Success Header */}
        ...
        <div className="bg-slate-900/50 rounded-lg p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-white text-sm font-mono">
              {deployResult.deploy.id.substring(0, 8)}...
            </span>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 rounded-lg p-3">
          ...
          <div className="flex items-center justify-between bg-slate-800 rounded px-3 py-2">
            <code className="text-blue-300 text-sm">
              {deployResult.site.url}
            </code>
            <button
              onClick={() =>
                navigator.clipboard.writeText(deployResult.site.url)
              }
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {/* Usage Monitor */}
      {showUsageMonitor && <NetlifyUsageMonitor />}
      ...
      <div className="rounded-lg border border-slate-700 bg-slate-800 p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Será usado como subdomínio:{' '}
            <code className="text-blue-400">
              {customSiteName || 'nome'}.netlify.app
            </code>
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            {htmlContent.length.toLocaleString()} caracteres • Arquivo HTML
            completo
          </p>
        </div>
        ...
        <div className="rounded-lg bg-blue-900/30 border border-blue-700 p-3">
          <p className="text-sm text-blue-300">
            💡 <strong>Grátis:</strong> Até 500 sites, 100GB bandwidth/mês, 300
            min build/mês. Seu site estará disponível instantaneamente em uma
            URL .netlify.app!
          </p>
        </div>
      </div>
