...
type OnboardingModalProps = {
  ...
  initialStep?: number; // Start on a specific step (0-2), used for payment-only flow
};
...
export function OnboardingModal({
  ...
  const handleNameChange = (value: string) => {
    ...
    if (value.trim().split(' ').length < 2) {
      setErrors((prev) => ({ ...prev, name: t('onboarding.fields.fullNameError') }));
    } else {
  ...
  const handlePhoneChange = (value: string) => {
    ...
    if (country && numbers.length < country.phoneLength) {
      setErrors((prev) => ({ ...prev, mobile_phone: t('onboarding.fields.phoneIncomplete') }));
    } else {
  ...
  // Filter languages (languages imported from i18n)
  const filteredLanguages = languages.filter((lang) =>
    lang.name.toLowerCase().includes(languageSearch.toLowerCase()),
  );

  // Handler for language change - also changes app language
  const handleLanguageChange = (languageCode: string) => {
    setFormData((prev) => ({ ...prev, preferred_language: languageCode }));
    changeLanguage(languageCode); // Change app language immediately
  ...
  const canProceed = (): boolean => {
    switch (currentStep) {
      case 0: // Personal Data + Terms + Language
        return !!(
          formData.name &&
      ...
      case 1: {
        ...
        // For Brazil, require state and city. For other countries, only require phone.
        const locationValid = isBrazil ? !!(formData.state && formData.city) : true;
        return !!(
          country &&
      ...
      case 2: // Plan Selection - always valid, handled by PlanSelectionStep
        return true;
      default:
        return true;
    }
  ...
  const nextStep = async () => {
    ...
    // Save user data after contact step (step 1) before going to plan selection
    if (currentStep === 1) {
      setLoading(true);
      try {
        const country = getCountryConfig(formData.country_code || 'BR');
        ...
        if (!result.success) {
          ...
          toast.error(t('onboarding.errors.saveError'), {
        ...
        toast.error(t('onboarding.errors.unexpectedError'), {
          description: t('onboarding.errors.tryAgain'),
        });
        setLoading(false);
        return;
            ...
            <label className="block text-left">
              ...
              <div className="relative" ref={languageDropdownRef}>
                ...
                {showLanguageDropdown && (
                  <div
                    className={`absolute z-50 mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 shadow-xl transition-opacity duration-700 ${isClosingLanguageDropdown ? 'opacity-0' : 'opacity-100'}`}
                  >
                    <div className="p-2 border-b border-slate-700">
                      <input
                        ...
                        placeholder={t('onboarding.fields.searchLanguage')}
      ...
      case 1: {
        ...
        return (
          <div className="space-y-4">
            <label className="block text-left">
              ...
              <div className="flex gap-2">
                ...
                <div className="relative" ref={countryDropdownRef}>
                  ...
                  {showCountryDropdown && (
                    <div
                      className={`absolute z-50 mt-1 w-80 rounded-lg border border-slate-700 bg-slate-800 shadow-xl transition-opacity duration-700 ${isClosingCountryDropdown ? 'opacity-0' : 'opacity-100'}`}
                    >
                      <div className="p-2 border-b border-slate-700">
                        <input
                          type="text"
                          ...
                          placeholder={t('onboarding.fields.searchCountry')}
                          className="w-full px-3 py-2 rounded-md bg-slate-700 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                          onClick={(e) => e.stopPropagation()}
                        />
                      </div>
            ...
            {/* State and City Dropdowns - Only shown for Brazil */}
            {isBrazil && (
              <>
                {/* State Dropdown */}
                <label className="block text-left">
                  <span className="mb-1 block text-sm font-medium text-slate-300">
                    {t('onboarding.fields.state')} *
                  </span>
                  <div className="relative" ref={stateDropdownRef}>
                    ...
                    >
                      <span className="text-slate-100 flex-1">
                        {selectedState
                          ...
                          : t('onboarding.fields.selectState')}
                      </span>
                      <svg
                        className="w-4 h-4 text-slate-400"
                        fill="none"
                    ...
                    {showStateDropdown && (
                      <div
                        className={`absolute z-50 mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 shadow-xl transition-opacity duration-700 ${isClosingStateDropdown ? 'opacity-0' : 'opacity-100'}`}
                      >
                        <div className="p-2 border-b border-slate-700">
                          <input
                            type="text"
                            ...
                            placeholder={t('onboarding.fields.searchState')}
                            className="w-full px-3 py-2 rounded-md bg-slate-700 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            onClick={(e) => e.stopPropagation()}
                          />
                        </div>
                ...
                <label className="block text-left">
                  ...
                  <div className="relative" ref={cityDropdownRef}>
                    ...
                    >
                      <span className="text-slate-100 flex-1">
                        {formData.city || (formData.state ? t('onboarding.fields.selectCity') : t('onboarding.fields.selectStateFirst'))}
                      </span>
                      <svg
                        className="w-4 h-4 text-slate-400"
                        fill="none"
                    ...
                    {showCityDropdown && formData.state && (
                      <div
                        className={`absolute z-50 mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 shadow-xl transition-opacity duration-700 ${isClosingCityDropdown ? 'opacity-0' : 'opacity-100'}`}
                      >
                        <div className="p-2 border-b border-slate-700">
                          <input
                            type="text"
                            ...
                            placeholder={t('onboarding.fields.searchCity')}
                            className="w-full px-3 py-2 rounded-md bg-slate-700 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            onClick={(e) => e.stopPropagation()}
                          />
                        </div>
                        <div className="max-h-60 overflow-y-auto">
                          {filteredCities.length > 0 ? (
                            ...
                            <div className="px-4 py-3 text-slate-400 text-sm">
                              {t('onboarding.fields.noCityFound')}
                            </div>
                          )}
                        </div>
                      </div>
      ...
      try {
        ...
        if (!result.success || !result.data) {
          ...
          setFormData((prev) => ({
            ...
            country_code: data.country || data.country_code || 'BR',
            state: data.state || '',
            city: data.city || '',
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation - Hide on step 2 (plan selection) since it has its own button */}
        {currentStep !== 2 && (
