 ...
 *
 * Compact certificate card with click-to-expand details.
...
import type { CertificateTier, CertificateCategory } from '../../types/trophies';
import { TIER_COLORS, getTierIndex } from '../../types/trophies';

interface CertificateTierInfo {
...
interface CertificateCardProps {
  ...
  progress: number;
  tiers: [CertificateTierInfo, CertificateTierInfo, CertificateTierInfo];
  compact?: boolean;
}

// Icon mapping
const CERTIFICATE_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  Flame,
  GraduationCap,
  CalendarCheck,
  Star,
...
// Tier badge colors
const TIER_BADGE_COLORS: Record<Exclude<CertificateTier, 'none'>, { bg: string; text: string; border: string }> = {
  bronze: {
    bg: 'bg-amber-900/40',
    text: 'text-amber-600',
...
const CertificateDetailModal = memo(function CertificateDetailModal({
  name,
  icon,
  category,
  currentTier,
  ...
  const Icon = CERTIFICATE_ICONS[icon] || Award;
  ...
  return (
    <>
      <button
        ...
        className={`relative w-full p-3 rounded-xl ${tierColors.bg} border ${tierColors.border} transition-all hover:scale-[1.02] hover:brightness-110 text-left group cursor-pointer`}
      >
        <div className="flex items-center gap-3">
          {/* Icon */}
